...
export default function AppointmentPage() {
  ...
  const [slotInfo,      setSlotInfo]     = useState({}); // { '08:00': { bookedCount, capacity, blocked, full } }
  ...
  const loadAvailability = (d) => {
    ...
    return appointmentsApi.availability(d)
        .then(res => {
          const map = {};
          (res.slots || []).forEach(s => {
            const key = s.time.length === 5 ? s.time : s.time.slice(0, 5); // 'HH:mm:ss' -> 'HH:mm'
            map[key] = s;
          });
          setSlotInfo(map);
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Appointment booked! A confirmation email is on its way — check your dashboard for details.');
      setSelectedTime('');
      setReason('');
  ...
  const isFull        = slot => !!slotInfo[slot]?.full;
  const isBlocked      = slot => !!slotInfo[slot]?.blocked;
  const isUnavailable = slot => isFull(slot) || isBlocked(slot) || isPastSlot(slot, date);

  const slotSubLabel = slot => {
    if (isPastSlot(slot, date)) return 'Passed';
    if (isBlocked(slot))        return 'Unavailable';
    if (isFull(slot))           return 'Fully booked';
    const info = slotInfo[slot];
    if (info && info.bookedCount > 0) {
      const left = info.capacity - info.bookedCount;
  ...
  return (
      <div style={{ minHeight: 'calc(100vh - var(--navbar-h))', background: 'var(--gray-50)' }}>
        ...
        <section className="section">
          <div className="container">
            <div className="layout-2col layout-2col--main-wide">
