...
import za.ac.vut.mavuti.dto.AppointmentDtos;
...
public interface AiHealthService {
    ...
    interface AppointmentService {
        ...
        AppointmentDtos.AppointmentResponse book(Long userId, AppointmentDtos.BookAppointmentRequest request);

        List<AppointmentDtos.AppointmentResponse> findMyAppointments(Long userId);
         ...
         */
        List<AppointmentDtos.SlotAvailability> getSlotAvailability(LocalDate date);

        /** Staff-only: full schedule for a given date, across all patients. */
        List<AppointmentDtos.AppointmentResponse> findByDate(LocalDate date);

        /** Staff-only: transition an appointment's status (confirm/cancel/complete/no-show). */
        AppointmentDtos.AppointmentResponse updateStatus(Long appointmentId, AppointmentStatus status);

        /** Staff/admin-only: mark a single hourly slot unavailable (e.g. a meeting). */
        AppointmentDtos.BlockedSlotResponse blockSlot(String blockedByInstitutionNumber, AppointmentDtos.BlockSlotRequest request);

        /** Staff/admin-only: mark every hourly slot in [startTime, endTime) unavailable in one call. */
        List<AppointmentDtos.BlockedSlotResponse> blockSlotRange(String blockedByInstitutionNumber, AppointmentDtos.BlockSlotRangeRequest request);
        ...
        List<AppointmentDtos.BlockedSlotResponse> listBlockedSlots(LocalDate date);
    }
}
